...
struct ClosestPolygonPoint
{
    ...
    ConstPolygonRef poly; //!< Polygon in which the result was found
...
public:
    ...
    */
     ...
     * \return The point on the polygon closest to \p from
     */
    static ClosestPolygonPoint moveInside2(ConstPolygonRef polygon, Point& from, const int distance = 0, const int64_t max_dist2 = std::numeric_limits<int64_t>::max(), const std::function<int(Point)>& penalty_function = no_penalty_function);
    ...
    */
     ...
     */
    static std::vector<std::pair<ClosestPolygonPoint, ClosestPolygonPoint>> findClose(ConstPolygonRef from, const Polygons& destination, const SparseGrid< PolygonsPointIndex >& destination_loc_to_line, const std::function<int(Point)>& penalty_function = no_penalty_function);
